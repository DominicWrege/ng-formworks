...
export class JsonPointer {
			...
			for (const keyItem of keyArray) {
				...
				if (isMap(subObject) && subObject.has(key)) {
					...
					if (evaluatedExpression.passed) {
						subObject = evaluatedExpression.key
							? subObject[evaluatedExpression.key]
							: subObject;
					} else {
	...
	static evaluateExpression(subObject: Object, key: any) {
		...
		const evaluatedResult = this.performExpressionOnValue(
			keysAndExpression,
			cleanedValue,
			subObject,
		);
	...
	private static performExpressionOnValue(
		...
		if (
			this.doComparisonByExpressionType(
				keysAndExpression.expressionType,
				propertyByKey,
				cleanedValue,
			)
		) {
	...
	private static doOwnCheckResult(
		subObject: Object,
		keysAndExpression: { expressionType: ExpressionType; keyAndValue: string[] },
	) {
	...
	static isSubPointer(
		...
		longPointer: Pointer | null | undefined,
	...
	static toIndexedPointer(
		genericPointer: Pointer | null | undefined,
		indexArray: number[],
		arrayMap: Map<string, number> | null = null,
	): string | undefined {
	...
	static toGenericPointer(
		indexedPointer: Pointer | null | undefined,
		arrayMap = new Map<string, number>(),
	): string | undefined {
		...
		if (dataPointerArray !== null) {
			for (const key of dataPointerArray) {
				...
				if (isArray(subGroup) && key === "-") {
					...
					console.error(
						`toControlPointer error: Unable to find "${key}" item in FormGroup.`,
					);
	...
	static toSchemaPointer(
		...
		if (this.isJsonPointer(dataPointer) && typeof schema === "object") {
			...
			console.error(
				`toSchemaPointer error: Data pointer ${dataPointer} ` +
	...
	static toDataPointer(
		schemaPointer: Pointer | null | undefined,
		schema: PlainObject,
		errors = false,
	): string | null {
		if (
