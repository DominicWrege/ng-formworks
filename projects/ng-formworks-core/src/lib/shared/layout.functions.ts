 ...
 * //
 */
export function buildLayout_original(
			...
			if (!hasOwn(newNode.options!, "validationMessages")) {
				if (hasOwn(newNode.options!, "errorMessages")) {
					...
					if (typeof newNode.options!.validationMessage === "string") {
						newNode.options!.validationMessages = newNode.options!
							.validationMessage as unknown as ValidationMessages;
					} else {
...
export function fixNestedArrayLayout(options: {
	...
	const traverseObj = function (
		obj: DataObject | unknown[],
		path: string,
		onValue?: (value: unknown, path: string) => void,
	) {
		if (isArray(obj)) {
	...
	const rebuildLayout = (options: {
		...
		if (
			builtLayout.items! &&
			dataTypes.indexOf(builtLayout.dataType!) >= 0 &&
			...
			if (numActualItems < Math.max(numDataItems, numListItems)) {
				...
				for (let i = 0; i < numItemsNeeded; i++) {
					...
					const isRefNode =
						builtLayout.items[0].type && builtLayout.items[0].type == "$ref";
...
export function buildLayoutFromSchema(
	jsf: JsonSchemaFormService,
	widgetLibrary: WidgetLibraryService,
	nodeValue: DataNodeValue = null,
	schemaPointer = "",
	dataPointer = "",
	arrayItem = false,
	arrayItemType: "tuple" | "list" | null = null,
	...
	function applyITEConditions(
		builtLayout: LayoutNode | LayoutNode[],
		schPointer: string,
		keySchemaPointer: string,
		negateClause: boolean,
		parentLayout?: LayoutNode | LayoutNode[],
	): void {
		if (builtLayout) {
			const parentCondition =
				parentLayout &&
				!isArray(parentLayout) &&
				parentLayout.isITEItem &&
				parentLayout.options?.condition;
			if (parentCondition) {
				return;
			}
			if (isArray(builtLayout)) {
				builtLayout.forEach((item) => {
					item.isITEItem = true;
					item.options!.condition = parentCondition
						? (null as unknown as string)
						: convertJSONSchemaIfToCondition(schema, item, negateClause);
					applyITEConditions(
						item,
						schPointer,
						keySchemaPointer,
				...
				builtLayout.options!.condition = parentCondition
					? (null as unknown as string)
					: convertJSONSchemaIfToCondition(schema, builtLayout, negateClause);
			}
		}
	}
	...
	const recursive =
		!shortDataPointer.length || shortDataPointer !== dataPointerPrefix + dataPointer;
	if (!jsf.dataMap.has(shortDataPointer)) {
		jsf.dataMap.set(shortDataPointer, new Map());
	}
	...
	if (newNode.dataType === "object") {
		...
		if (isObject(schema.properties)) {
			...
			propertyKeys
				.filter(
					(key) =>
						hasOwn(schema.properties, key) || hasOwn(schema, "additionalProperties"),
				)
				.forEach((key) => {
					...
					const innerItem = buildLayoutFromSchema(
						jsf,
						widgetLibrary,
						isObject(nodeValue) ? (nodeValue as DataObject)[key] : null,
				...
				if (hasOwn(schema, ofType) && isArray(schema[ofType])) {
					...
					if (ofType == "oneOf") {
						outerOneOfItem = buildLayoutFromSchema(
							jsf,
							widgetLibrary,
							schema.oneOf, //{type:"tabarray",items:schema.oneOf},
							...
							if (!isArray(innerItem) && innerItem.items) {
								innerItem.items.forEach((innerItemLevel2) => {
									const l2SchemaPointer = hasOwn(ofItem, "properties")
										? "/properties/" + innerItemLevel2.name
										: innerItemLevel2.name;
									...
									const ofPointerKeys: Record<string, string> = {
										anyOf: "anyOfPointer",
										oneOf: "oneOfPointer",
									};
									const ofPointer = ofPointerKeys[ofType];
									innerItemLevel2[ofPointer] = ofPointer
		...
		if (isArray(schema.items)) {
			...
			for (let i = 0; i < newNode.options!.tupleItems; i++) {
				...
				const itemRecursive =
					!itemRefPointer.length || itemRefPointer !== shortDataPointer + "/" + i;
				...
				if (removable && i >= newNode.options!.minItems) {
					...
					if (!hasOwn(jsf.layoutRefLibrary, itemRefPointer)) {
						...
						jsf.layoutRefLibrary[conditionalRefPointer] =
							jsf.layoutRefLibrary[itemRefPointer];
					}
		...
		if (newNode.options!.add) {
			...
			const parentSchema: JsonSchema = JsonPointer.get(jsSchema, schemaPointer, 0, -1);
			if (hasOwn(parentSchema, "title")) {
		...
		if (isNumber(JsonPointer.get(jsSchema, schemaPointer, 0, -1).maxItems)) {
			newNode.options!.maxItems = JsonPointer.get(jsSchema, schemaPointer, 0, -1).maxItems;
		}
...
export function mapLayout(
	layout: DataObject | unknown[],
	fn: (v: any, i?: number, l?: any, p?: any) => any,
	layoutPointer = "",
	rootLayout: DataObject | unknown[] = layout,
): LayoutNode[] {
	let indexPad = 0;
...
export function getLayoutNode(
	refNode: LayoutNode,
	jsf: JsonSchemaFormService,
	widgetLibrary: WidgetLibraryService | null = null,
	nodeValue: DataObject | null = null,
): LayoutNode {
...
export function buildTitleMap(
	titleMap: TitleMapItem[] | DataObject | null,
	enumList: unknown,
	fieldRequired = true,
	flatList = true,
): TitleMapItem[] {
