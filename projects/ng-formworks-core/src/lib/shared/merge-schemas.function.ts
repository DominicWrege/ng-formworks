				...
				switch (key) {
						...
						if (isObject(combinedValue) && isObject(schemaValue)) {
							...
							for (const subKey of Object.keys(schemaValue)) {
								if (
									...
									isArray(schemaValue[subKey]) &&
									isArray(combinedObject[subKey])
								) {
									combinedObject[subKey] = uniqueItems(
					...
					case 'items':
						...
						if (isArray(combinedValue) && isArray(schemaValue)) {
							...
							combinedSchema.items = combinedValue.map((item) =>
								mergeSchemas(item, schemaValue),
							);
						} else if (isObject(combinedValue) && isArray(schemaValue)) {
							combinedSchema.items = schemaValue.map((item) =>
								mergeSchemas(item, combinedValue),
							);
						} else {
							return { allOf: [...schemas] };
						}
					...
					case 'multipleOf':
						...
						if (isNumber(combinedValue) && isNumber(schemaValue)) {
							...
							combinedSchema.multipleOf = lcm(
								combinedValue as number,
								schemaValue as number,
							);
						} else {
							return { allOf: [...schemas] };
						}
						...
						if (isObject(combinedValue) && isObject(schemaValue)) {
							...
							if (hasOwn(schemaValue, 'additionalProperties')) {
								Object.keys(combinedValue)
									.filter(
										(combinedKey) =>
											!Object.keys(schemaValue).includes(combinedKey),
									)
							...
							for (const subKey of Object.keys(schemaValue)) {
								if (
									...
									if (isObject(combinedObject.additionalProperties)) {
										combinedObject[subKey] = mergeSchemas(
											combinedObject.additionalProperties,
											schemaValue[subKey],
										);
									}
									// If both keys are objects, merge them
