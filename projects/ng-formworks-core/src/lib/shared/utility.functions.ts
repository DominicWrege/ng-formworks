...
export function copy<T>(object: T, errors = false): T {
	...
	if (isMap(object)) {
		return new Map(object) as T;
	}
	if (isSet(object)) {
		return new Set(object) as T;
	}
	if (isArray(object)) {
		return [...object] as T;
	}
...
export function commonItems(...arrays: (string | string[])[]): string[] {
	...
	for (const array of arrays) {
		...
		returnItems =
			returnItems === null ? [...items] : returnItems.filter((item) => items.includes(item));
	...
	if (isObject(obj1) && !isArray(obj1)) {
		...
		for (let key in record1) {
			if (record2.hasOwnProperty(key)) {
				const result = compareObjectArraySizes(
					record1[key],
					record2[key],
					`${comparePath}/${key}`,
				);
...
class ConditionParser {
	...
	static extractDependencies(params: any[]): string[] {
		...
		params.forEach((param) => {
			...
			const regex =
				/model\.[a-zA-Z0-9_]+\[(.*)\]\.[a-zA-Z0-9_]+|model\.[a-zA-Z0-9_]+\.[a-zA-Z0-9_]+/g;
