...
export class JsonValidators {
	...
	static composeAnyOf(validators: IValidatorFn[]): IValidatorFn {
		...
		return (control: AbstractControl, invert = false): ValidationErrors | null => {
			const arrayOfErrors = _executeValidators(control, presentValidators, invert).filter(
				isDefined,
			);
	...
	static composeAsync(validators: AsyncIValidatorFn[]): AsyncIValidatorFn {
		...
		return (control: AbstractControl) => {
			const observables = _executeAsyncValidators(control, presentValidators).map(
				toObservable,
			);
			return (
				map as unknown as (
					project: (
						arrayOfErrors: (PlainObject | null | undefined)[],
					) => PlainObject | null,
				) => Observable<PlainObject>
			).call(forkJoin(observables), _mergeErrors);
		};
	}
