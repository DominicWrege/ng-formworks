		...
		if (this.layoutNode()!.items) {
			this.layoutNode()!.items!.forEach((layoutItem, ind) => {
				...
				if (layoutItem.oneOfPointer) {
					...
					if (
						hasOwn(this.jsf.formGroup!.controls, controlKey) &&
						(formValue ||
							hasNonNullValue(this.jsf.formGroup!.controls[controlKey].value))
					...
					if (foundInd == -1) {
						...
						let descendantOneOfControlNames = Object.keys(
							this.jsf.formGroup!.controls,
						).filter((controlName) => {
						...
						descendantOneOfControlNames.forEach((controlName) => {
							...
							if (controlSchema && controlSchema.properties) {
								compareVal =
									isObject(dVal) && hasOwn(dVal, fieldName)
										? pick(
												dVal[fieldName],
												Object.keys(controlSchema.properties),
											)
